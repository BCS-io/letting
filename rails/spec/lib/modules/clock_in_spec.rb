...
RSpec.describe ClockIn do
  it 'defaults booking_date to today' do
    expect(described_class.new.booking_date).to eq Time.zone.today
  end
  ...
  describe '#recorded_as' do
    describe '#initialize add_time' do
      ...
      it 'leaves off if not required' do
        ...
        time = described_class.new.recorded_as booked_time: today,
                                               add_time: false
    ...
    it 'recorded as now when booked_time today' do
      time = described_class.new.recorded_as booked_time: Time.zone.today + 1.hour
