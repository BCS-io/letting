...
RSpec.describe AccountCreditDecorator do
  describe 'accessors for' do
    it '#charge_type' do
      credit_dec = described_class.new credit_new(at_time: '10/6/2014')
    ...
    it '#due' do
      expect(described_class.new(credit_new).due).to eq ''
    end

    it '#payment' do
      credit_dec = described_class.new credit_new(amount: 90.15)
      expect(credit_dec.payment).to eq '90.15'
    end

    it '#amount' do
      credit_dec = described_class.new credit_new(amount: 32.05)
